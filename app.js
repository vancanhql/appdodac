...
function batGPS() {
  ...
  alert("🟢 Bắt đầu đo GPS – vui lòng di chuyển quanh ranh lô");
  ...
  gpsWatch = navigator.geolocation.watchPosition(p => {
    ...
    if (gpsLiveMarker) map.removeLayer(gpsLiveMarker);
    gpsLiveMarker = L.circleMarker(latlng, {
      radius: 6,
      color: "red",
      fillColor: "red",
      fillOpacity: 1
    ...
    drawn.clearLayers();
    L.polyline(gpsPts, { color: "red", weight: 3 }).addTo(drawn);
